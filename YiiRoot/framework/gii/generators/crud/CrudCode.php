...
class CrudCode extends CCodeModel
{
           ...
           public function generateActiveField($modelClass,$column)
        {
			...
			return "\$form->checkBox(\$model,'{$column->name}')";
                ...
                if(stripos($column->dbType,'bigint')!==false && substr($column->name, strlen($column->name) - 2,2) == 'ID')
                {
                    ...
                    $Name = 'Name';
                    return "CHtml::dropDownList('"  . $modelClass . "[" . $column->name . "]', \$model->" . $column->name . ", CHTML::listData(" . $ModelName . "::model()->findAll(array('order'=>'Name')), 'id" . $ModelName . "', '" . $Name . "'), array('prompt' => 'Select a " . $ModelName . "'))";
                }    
                else if($column->dbType == 'date')
                    return "\$form->dateField(\$model,'{$column->name}')";
                else if($column->dbType == 'time')
                    return "\$form->timeField(\$model,'{$column->name}')";       
                else if($column->dbType == 'int')
                    return "\$form->numberField(\$model,'{$column->name}')";   
                else if($column->name == 'Phone' || $column->name == 'Fax')
                    return "\$form->telField(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))"; 
                else if($column->name == 'Email')
                    return "\$form->emailField(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))"; 
                else if($column->name == 'Web')
                    return "\$form->urlField(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))"; 
	...
	public function generateActiveFieldOLD($modelClass,$column)
	{
		if($column->type==='boolean')
			return "\$form->checkBox(\$model,'{$column->name}')";
		elseif(stripos($column->dbType,'text')!==false)
